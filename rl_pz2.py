...
#Parameters
env = gym.make('gym_tdw:tdw_puzzle_2-v0')
...
def transform_action(action):
    multiplier = 40/grid
    return {'x':(action//grid)*multiplier-20,'z':(action%grid)*multiplier-20}
...
def aux_reward(state,red_ball,ball1,ball2,ball1_hit,ball2_hit):
    ...
    if red_ball is None:
        for k, v in state['object_information'].items():
            if v['model_name'] == 'prim_sphere' and v['velocity']['x']!=0:
                red_ball = k

    if red_ball is not None and ball1 is None:
        for k, v in state['object_information'].items():
            if v['model_name'] == 'prim_sphere' and v['velocity']['x']==0:
                if ball1 is None:
                    ball1 = k
                    ...
                    ball2 = k
    if red_ball is None or ball1 is None or ball2 is None:
        return 0
    pos = []
    pos.append(vel(state['object_information'][red_ball]))
    pos.append(vel(state['object_information'][ball1]))
    pos.append(vel(state['object_information'][ball2]))

    dist1 = distance.euclidean(pos[0],pos[1])
    dist2 = distance.euclidean(pos[0],pos[2])
    ...
    for i_episode in count(episodes):
        ...
        for t in range(100):
            ...
            if done:
                ...
                finished = finished + 1
                reward = reward - 5
            ar, closer = aux_reward(state,red_ball,ball1,ball2,ball1_hit,ball2_hit)
            reward = reward + ar
            if reward < -100:
                done = True
            state = state['image_1']
        ...
        if i_episode % 1 == 0:
            ...
            pickle.dump(rewards,open('AC_rewards_pz2.p','wb'))
            torch.save(model.state_dict(),'AC_model_pz2')
        finish_episode()
        if finished > 5:
            steps = 100
        final_reward.append(steps)
        pickle.dump(final_reward,open('AC_steps_pz2.p','wb'))
        #env.close()
